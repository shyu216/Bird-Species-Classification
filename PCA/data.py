...
def get_clip_names(test=False):
    ...
    if test:
        dir = os.path.join(config.WAV_DIR, "train")
        test_set = config.TEST_SET
        with open(test_set) as f:
            # original :
            # data/NIPS4B_BIRD_CHALLENGE_TRAIN_TEST_WAV/train/cepst_conc_cepst_nips4b_birds_trainfile095.txt
            # to:
            # data/NIPS4B_BIRD_CHALLENGE_TRAIN_TEST_WAV/train/nips4b_birds_trainfile004.wav
            return [line.strip().replace("txt", "wav").replace("cepst_conc_cepst_nips4b_birds_", "nips4b_birds_") for line in f.readlines()]   
    else:
        dir = os.path.join(config.WAV_DIR, "train")
